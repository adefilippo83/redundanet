...
class TestFingerprintMatching:
    ...
    def test_short_ids_match_as_suffix(self, armored_key_and_fpr):
        armored, fpr = armored_key_and_fpr
        assert armored_key_matches_id(armored, fpr[-16:])
        assert armored_key_matches_id(armored, f"0x{fpr[-8:]}")
...
class TestFetchKeyPinning:
    ...
    def test_matching_key_is_returned(self, armored_key_and_fpr):
        ...
        assert client.fetch_key(fpr[-16:]) == armored
    ...
    def test_mismatched_key_is_discarded(self, armored_key_and_fpr):
        ...
        a lookup by the victim's (short) id returns attacker material.
        """
        armored, _fpr = armored_key_and_fpr
        client = self._client_returning(armored)
        assert client.fetch_key("0000000000000000") is None
    ...
    def test_non_key_response_is_ignored(self):
        ...
        assert client.fetch_key("DEADBEEFCAFE1234") is None
