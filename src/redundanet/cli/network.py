...
def _find_node_in_manifest(manifest_path: Path, node_name: str) -> dict | None:
...
def _list_nodes_in_manifest(manifest_path: Path) -> list[dict]:
...
def _generate_env_file(node: dict, repo_url: str, branch: str, install_dir: Path) -> None:
...
def join_network(
    ...
    console.print(
        "1. Export GPG key: [cyan]gpg --armor --export-secret-keys <KEY_ID> > /opt/redundanet/docker/secrets/gpg_private_key.asc[/cyan]"
    )
    console.print(
