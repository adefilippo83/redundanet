...
> **Note:** the containers need the node's environment file
> (`/opt/redundanet/.env` — node name, VPN IP, GPG key path...), which is
> generated by `redundanet network join`. See the
> [Quick Start Guide](quickstart.md) for the full join flow.
